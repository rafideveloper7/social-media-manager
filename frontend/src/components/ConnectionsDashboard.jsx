...
const ConnectionsDashboard = ({ userId }) => {
  ...
  const handlePublish = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://smmb.vercel.app/api/posts', formData, {
        headers: {
      ...
      alert(`Pipeline rejection: ${error.response?.data?.message || error.message}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <Layers className="w-6 h-6 text-indigo-600" /> Multi-Account Publishing Studio
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Running 6 connected accounts utilizing 3 multi-tenant free developer access endpoints.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <h2 className="text-lg font-bold text-slate-800 mb-4">Active Managed Channels</h2>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="bg-slate-100 p-2 rounded-lg flex items-center justify-center">{social.icon}</div>
                      <div>
                        <p className="font-semibold text-slate-700 text-sm">{social.name}</p>
                        <p className="text-[11px] text-slate-400 font-medium">{social.keyGroup}</p>
                      </div>
                    </div>
        ...
        <div className="lg:col-span-2">
          <form onSubmit={handlePublish} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-5">
            <h2 className="text-lg font-bold text-slate-800">Compose Broadcast Campaign</h2>

            <div>
              <label className="block text-sm font-semibold text-slate-600 mb-1">Campaign Title</label>
              <input
                type="text"
                value={title}
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-600 mb-1">Description / Post Body</label>
              <textarea
                value={description}
            ...
            {/* Drag & Drop Media Box */}
            <div>
              <label className="block text-sm font-semibold text-slate-600 mb-1">Media Asset Upload</label>
              <div className="border-2 border-dashed border-slate-200 hover:border-slate-300 transition-colors rounded-xl p-6 text-center relative bg-slate-50/50">
                ...
                {!previewUrl ? (
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500 font-medium">Click or Drag Image/Video file here</p>
                    <p className="text-[10px] text-slate-400">Cloudinary handles secure caching auto-processing</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {mediaFile?.type.startsWith('video/') ? (
                      ...
                      <img src={previewUrl} alt="Upload Preview" className="max-h-40 mx-auto rounded-lg object-contain" />
                    )}
                    <p className="text-xs font-semibold text-slate-600">{mediaFile?.name}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            >
              {loading ? 'Executing Multi-Key Blaster Pipelines...' : 'Publish Media Update Everywhere Now'}
            </button>
          </form>
        </div>
